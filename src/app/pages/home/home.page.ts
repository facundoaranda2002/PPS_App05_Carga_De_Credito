...
export class HomePage implements OnInit {
    ...
    this.firestoreUser.getUsers().subscribe((users) => {
      ...
      users.forEach((user) => {
        if (user.email == this.authService.currentUserSig()?.email) {
          this.creditos = user.cantidadCreditos;
        }
      });
    });
  }
      ...
      this.usuarios?.forEach((user) => {
        if (user.email == this.authService.currentUserSig()?.email) {
          ...
          if (user.perfil == 'admin') {
            ...
            if (ocurrencias < 2) {
              //dar ok
              this.agregarCreditosCorrespondientes(user.id, user);
  ...
  agregarCreditosCorrespondientes(id: string, user: User) {
    let cantidadActualizada = user.cantidadCreditos;
    ...
    if (flag) {
      const userAux: User = {
        ...
        perfil: user.perfil,
        sexo: user.sexo,
        cantidadCreditos: cantidadActualizada,
